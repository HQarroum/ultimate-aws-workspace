...
In this section, we list the different packages, drivers and configuration specific items that this project deploys on EC2 instances. All those items are customizable through the Ansible role. We list the packages for Linux and Windows separately, as they both have different packages available.
